> 参考自[《设计模式之美》](https://time.geekbang.org/column/article/197254)
...
无论是简单工厂、工厂方法工厂抑或是抽象工厂，它们都只能对某一类对象进行生产制造，这样工厂类的数量就会随着需要被创建的不同种类的类的增多而增多。为了解决这个问题，可以采用反射工厂的手法来处理这个问题。
...
public class ReflectionFactory<T> {
    ...
    public<S extends T> T getInstance(Class<S> clazz){
        try {
    ...
    public <S extends T> T getInstance(Class<S> clazz,Object[] args) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        Constructor<S>[] constructors = (Constructor<S>[]) clazz.getConstructors();
