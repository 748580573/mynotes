...
> 本文摘自：https://time.geekbang.org/column/article/102931

## 背景

Kafka消息队列丢失数据一直是开发者需要在开发前避免的问题，数据的丢失往往会带来不可估量的损失，下面本文将介绍常见的Kafka消息队列数据丢失的处理方案。
...
正确使用书签有两个步骤：第一步是读书，第二步是更新书签页。如果这两步的顺序颠倒了，就可能出现这样的场景：当前的书签页是第 90 页，我先将书签放到第 100 页上，之后开始读书。当阅读到第 95 页时，我临时有事(消费者出故障)中止了阅读。那么问题来了，当我下次直接跳到书签页阅读时，我就丢失了第 96～99 页的内容，即这些消息就丢失了。
